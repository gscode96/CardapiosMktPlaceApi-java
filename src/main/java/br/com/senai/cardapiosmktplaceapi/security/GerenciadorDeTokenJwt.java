...
public class GerenciadorDeTokenJwt {
	...
	public String gerarTokenPor(String login) {
		...
		return Jwts.builder().setClaims(new HashMap<String, Object>()).setSubject(login)
	...
	private Claims extrairDetalhesDo(String tokenGerado) {
		return Jwts.parserBuilder().setSigningKey(getChaveDeAssinatura()).build().parseClaimsJws(tokenGerado).getBody();
