...
public interface CategoriasRepository extends JpaRepository<Categoria, Integer> {
	...
	@Query(value = "SELECT c FROM Categoria c WHERE Upper(c.nome) LIKE Upper(:nome) AND c.status = :status AND c.tipo = :tipo ORDER BY c.nome", 
			countQuery = "SELECT Count(c) FROM Categoria c WHERE Upper(c.nome) LIKE Upper(:nome) AND c.status = :status AND c.tipo = :tipo")
	public Page<Categoria> listarPor(String nome, Status status, TipoDeCategoria tipo, Pageable paginacao);

	@Modifying // informa que nao é uma consulta e sim uma modificação do banco
